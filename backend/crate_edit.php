<?php 
    ...
    if (isset($_REQUEST['update_id'])) {
        try {
            ...
            $select_stmt = $db->prepare("SELECT * FROM crate WHERE id = :id");
    ...
    if (isset($_REQUEST['btn_update'])) {
        ...
        if (empty($itemid)) {
            ...
            try {
                if (!isset($errorMsg)) {
                    $update_stmt = $db->prepare("UPDATE crate SET ItemID = :iitem, Amount = :iam, droprate = :idr, crateid = :icid WHERE id = :id");
                    $update_stmt->bindParam(':iitem', $itemid);
                    $update_stmt->bindParam(':iam', $amount);
                    $update_stmt->bindParam(':idr', $droprate);
                    $update_stmt->bindParam(':icid', $crateid);
                    $update_stmt->bindParam(':id', $id);

                    if ($update_stmt->execute()) {
                        $updateMsg = "เรียบร้อยละ";
                        header("refresh:2;crate.php");
                    }
                }
            } catch(PDOException $e) {
                echo $e->getMessage();
...
<body>
    ...
    <div class="display-3 text-center"><h3>Edit Crate</h3></div>
    ...
    <div class="form-group text-center">
                ...
                <label for="info" class="col-sm-3 control-label">ItemID</label>
                    ...
                    <div class="col-sm-9">
                        <input type="text" name="txt_am" class="form-control" value="<?php echo $Amount; ?>">
                    </div>
                    <br><br>
                    <label for="info" class="col-sm-3 control-label">Drop Rate</label>
                    <div class="col-sm-9">
                        <input type="text" name="txt_dr" class="form-control" value="<?php echo $droprate; ?>">
                    </div>
                    <br><br>
