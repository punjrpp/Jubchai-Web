<?php 
    ...
    if (isset($_REQUEST['btn_update'])) {
        ...
        try {
            $it_name = $_REQUEST['txt_name'];
            $it_code = $_REQUEST['txt_namecode'];
            $it_price = $_REQUEST['txt_price'];
            $it_sale = $_REQUEST['txt_sale'];
            ...
            $path = "upload_item/".$image_file;
            $directory = "upload_item/"; 

            if ($image_file) {
                if ($type == "image/jpg" || $type == 'image/jpeg' || $type == "image/png" || $type == "image/gif") {
                    if (!file_exists($path)) {
                        if ($size < 5000000) {
                            unlink($directory.$row['item_image']);
                            move_uploaded_file($temp, 'upload_item/'.$image_file);
                        } else {
                ...
                $image_file = $row['item_image'];
            }

            if(!isset($errorMsg)) {
                $update_stmt = $db->prepare("UPDATE web_shop_item SET item_image = :file_up, item_name = :item_name_up, item_code = :item_code_up, price = :price_up, sale = :sale_up WHERE id = :id");
                $update_stmt->bindParam(':file_up', $image_file);
                $update_stmt->bindParam(':item_name_up', $it_name);
                $update_stmt->bindParam(':item_code_up', $it_code);
                $update_stmt->bindParam(':price_up', $it_price);
                $update_stmt->bindParam(':sale_up', $it_sale);
                $update_stmt->bindParam(':id', $id);

                if($update_stmt->execute()) {
                    $updateMsg = "แก้ไขเรียบแล้วร้อยแล้วไอ่สัส";
                    header("refresh:2;shop_item.php");
                }
            }
...
<body>
    ...
    <div class="container">
        ...
        <a href="shop_item.php" class="btn btn-info">back</a>
        <?php 
         if (isset($errorMsg)) {
    ?>
        ...
        <form action="" method="post" class="form-horizontal backend-" enctype="multipart/form-data">
            ...
            <div class="form-payment">
                <div class="row">
                    ...
                    <div class="col-sm-9">
                        <input type="text" name="txt_name" class="form-control" value="<?php echo $item_name; ?>">
                    </div>
                </div>
            </div>
            <br>
            <div class="form-payment">
                <div class="row">
                    ...
                    <div class="col-sm-9">
                        <input type="text" name="txt_namecode" class="form-control" value="<?php echo $item_code; ?>">
                    </div>
                </div>
            </div>
            <br>
