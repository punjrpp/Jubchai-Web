<?php 
  ...
  if (isset($_REQUEST['delete_id'])) {
    ...
    $delete_stmt = $db->prepare('DELETE FROM web_shop_item WHERE id = :id');
  ...
  if (isset($_REQUEST['btn_shop'])) {
    try {
        ...
        if (!isset($errorMsg)) {
            ...
            $insert_stmt = $db->prepare('INSERT INTO web_shop_item (item_image, item_name, item_code, price, sale) 
                                        VALUES (:fitem_image, :fitem_name, :fitem_code, :fprice, :fsale)');
            $insert_stmt->bindParam(':fitem_image', $image_file);
            $insert_stmt->bindParam(':fitem_name', $name);
            $insert_stmt->bindParam(':fitem_code', $name_code);
...
<body>
            ...
            ?>
                    <div class="main">
                    <div class="img-shop">
                        <img src="upload_item/<?php echo $it["item_image"] ?>" alt="item" class="logo-rank">

                    </div>
                    <div class="content">
                        <h1><?php echo $it["item_name"] ?></h1>
                        <p><?php echo "ราคา: " .$it["price"]. " พอยท์" ?></p>
                        <a href="member/buy.php" name="btn_buy" class="btn btn-success mb-3">ซื้อ</a>
                    </div>
                    <div class="footer">
                        <div class="ft-l">

                        <a href="item_edit.php?update_id=<?php echo $it['id']; ?>" class="badge bg-warning">Edit</a> &nbsp;&nbsp;
                        <a href="?delete_id=<?php echo $it['id']; ?>" class="badge bg-danger">Delete</a>
                        </div>
                    </div>
                </div>
                <?php } ?>
